...
function adaptLinks(dom, blobRoot) {
  dom.find('.tag-source li')
    .map(function (idx, x) {
        if (x.children && x.children[1] && x.children[3]) {
          return {
            li: cheerio(x),
            file: x.children[1].attribs.href.replace('.html','').trim(),
            lineno: x.children[3].attribs.href.replace(/.*#line/,'').trim()
          };
        }
    })
    .filter(function (x) { return x })
    .forEach(function (x) {
      var fileUrl = blobRoot + '/lib/' + x.file;
      x.li.replaceWith(
        '\n<li>\n' +
